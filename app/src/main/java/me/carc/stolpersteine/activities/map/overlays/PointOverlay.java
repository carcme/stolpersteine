...
public class PointOverlay extends Overlay {
    private final PointOverlayOptions mStyle;
    private final PointOverlay.PointAdapter mPointList;
    private final BoundingBox mBoundingBox;
    private Integer mSelectedPoint;
    private PointOverlay.OnClickListener clickListener;
    private List<PointOverlay.StyledLabelledPoint> gridIndex;
    ...
    public interface OnClickListener {
        void onClick(PointOverlay.PointAdapter points, Integer point);
    }

    public PointOverlay(PointOverlay.PointAdapter pointList, PointOverlayOptions style) {
        mStyle = style;
    ...
    public PointOverlay(PointOverlay.PointAdapter pointList) {
        this(pointList, PointOverlayOptions.getDefaultStyle());
    }
        ...
        if (viewBBox.getLatNorth() != prevBoundingBox.getLatNorth()
            ...
            for (IGeoPoint pt1 : mPointList) {
                ...
                if (pt1.getLatitude() > viewBBox.getLatSouth()
                    ...
                    gridIndex.add(new PointOverlay.StyledLabelledPoint(mPositionPixels
    ...
    public boolean onTouchEvent(MotionEvent event, final MapView mapView) {
        if (mStyle.mAlgorithm !=
                PointOverlayOptions.RenderingAlgorithm.MAXIMUM_OPTIMIZATION) return false;
    ...
    public void setOnClickListener(PointOverlay.OnClickListener listener) {
    ...
    public void draw(Canvas canvas, MapView mapView, boolean b) {
        ...
        if (mStyle.mPointStyle != null || mPointList.isStyled()) {
            switch (mStyle.mAlgorithm) {
                case MAXIMUM_OPTIMIZATION:
                    ...
                    showLabels =
                            ((mStyle.mLabelPolicy == PointOverlayOptions.LabelPolicy.DENSITY_THRESHOLD
                                    && numLabels <= mStyle.mMaxNShownLabels)
                                    || (mStyle.mLabelPolicy == PointOverlayOptions.LabelPolicy.ZOOM_THRESHOLD
                                    && mapView.getZoomLevelDouble() >= mStyle.mMinZoomShowLabels));
                    // draw points
                    for (PointOverlay.StyledLabelledPoint slp : gridIndex) {
                ...
                case MEDIUM_OPTIMIZATION:
                    ...
                    showLabels = (mStyle.mLabelPolicy == PointOverlayOptions.LabelPolicy.ZOOM_THRESHOLD
                            && mapView.getZoomLevelDouble() >= mStyle.mMinZoomShowLabels);
        ...
        if (mSelectedPoint != null && mSelectedPoint < mPointList.size() &&
            ...
            if (mStyle.mSymbol == PointOverlayOptions.Shape.CIRCLE)
